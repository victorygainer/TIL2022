...
//	category.xml 파일의 select sql 명령을 실행하는 메소드
	public ArrayList<CategoryVO> selectList(SqlSession mapper) {
		...
		// selectOne(): ibatis의 queryForObject() 메소드와 같은 기능이 실행된다. => 결과가 1개인 select => Object 타입이 리턴된다.
		// selectList(): ibatis의 queryForList() 메소드와 같은 기능이 실행된다. => 결과가 여러개인 select => List 타입이 리턴된다.
		return (ArrayList<CategoryVO>) mapper.selectList("selectList");
	}
	
//	CategoryService 클래스에서 호출되는 mapper와 gup와 seq가 저장된 HashMap 객체를 넘겨받고 
//	같은 카테고리 그룹(gup)에서 조건에 만족하는 카테고리 출력 순서를 조정하기 위해 seq를 1씩
//	증가시키는 category.xml 파일의 update sql 명령을 실행하는 메소드
...
//	CategoryService 클래스에서 호출되는 mapper와 테이블에 저장할 서브 카테고리 정보가 저장된
//	객체를 넘겨받고 테이블에 서브 카테고리를 저장하는 category.xml 파일의 insert sql 명령을 실행하는 
//	메소드
...
//	CategoryService 클래스에서 호출되는 mapper와 서브 카테고리 번호를 넘겨받고 서브 카테고리
//	1건을 얻어오는 category.xml 파일의 select sql 명령을 실행하는 메소드
...
//	삭제하는 category.xml 파일의 delete sql 명령을 실행하는 메소드
...
//	deleteReport 필드를 1증가 시키는 category.xml 파일의 update sql 명령을 실행하는 메소드
...
//	명령을 실행하는 메소드
	public ArrayList<CategoryVO> selectGup(SqlSession mapper, int gup) {
		System.out.println("CategoryDAO 클래스의 selectGup() 메소드");
...
//	저장된 HashMap 객체를 넘겨받고 seq를 수정하는 category.xml 파일의 update sql 명령을 실행하는 메소드
	public void reSeq(SqlSession mapper, HashMap<String, Integer> hmap) {
		System.out.println("CategoryDAO 클래스의 reSeq() 메소드");
