...
//	package: 접근 권한 생략시, 같은 패키지에서는 public 처럼, 다른 패키지에서는 private 처럼 사용된다.
//	public: 프로젝트의 모든 클래스에서 자유롭게 접근할 수 있다.
	
//	정적(static) 메소드
//	static을 붙여서 선언한 메소드는 메소드를 선언한 클래스의 객체를 생성하지 않고 클래스 이름에 "."을
//	찍어서 바로 접근할 수 있다.
//	자주 사용하는 메소드는 static 메소드로 만들어 사용하면 객체를 선언하지 않고 사용할 수 있어 편리하다.
...
//		메소드의 인수를 배열로 구현하면 많은 수의 오버로딩이 필요 없지만 인수를 배열로 선언하고 선언한
//		배열에 초기치를 지정해서 호출되는 메소드로 전달해야하는 번거로움이 있다.
...
//	메소드 이름이 같으면 인수의 개수로 메소드를 식별하고 인수의 개수도 같다면 인수의 데이터 타입으로 메소드를
//	식별한다.
...
//	위 4개의 메소드는 인수를 모두 변수로 받고있기 때문에 많은 수의 메소드 오버로딩이 필요했다.
//	인수를 배열로 받으면 많은 수의 메소드 오버로딩이 필요없다.
