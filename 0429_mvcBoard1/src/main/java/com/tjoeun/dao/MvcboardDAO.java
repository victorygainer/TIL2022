...
//	MvcboardService 클래스에서 호출되는 mapper를 넘겨받고 테이블에 저장된 메인글 전체 개수를 얻어오는
...
//	MvcboardService 클래스에서 호출되는 mapper와 조회수를 증가시킨 글번호를 넘겨받고 조회수를 증가시킨
//	글 1건을 얻어오는 mvcboard.xml 파일의 select sql 명령을 실행하는 메소드
...
//	MvcboardService 클래스에서 호출되는 mapper와 삭제할 글번호를 글 1건을 삭제하는
...
//	MvcboardService 클래스에서 호출되는 mapper와 gup와 seq가 저장된 MvcboardVO 클래스 객체를
//	넘겨받고 조건에 만족하는 글의 seq를 1씩 증가시키는 mvcboard.xml 파일의 update sql 명령을
//	실행하는 메소드
	public void incrementSeq(SqlSession mapper, MvcboardVO vo) {
		System.out.println("MvcboardDAO 클래스의 incrementSeq() 메소드");
		mapper.update("incrementSeq", vo);
	}
	
//	MvcboardService 클래스에서 호출되는 mapper와 답글 정보가 저장된 MvcboardVO 클래스 객체를
//	넘겨받고 답글을 저장하는 mvcboard.xml 파일의 insert sql 명령을 실행하는 메소드
