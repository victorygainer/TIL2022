...
//	한글 깨짐을 방지하려면 최초의 request.getParameter() 메소드가 실행되기 전(맨처음)에 아래 내용을
//	코딩하면 된다.
	request.setCharacterEncoding("UTF-8");

//	get 방식도 한글이 깨졌었었다. => tomcat server 7.0 부터 get 방식의 한글이 깨지지 않는다.
//	get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야 한다. => Server 폴더의
//	server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8" 속성을 추가하면 된다. => 서버에
//	추가하기 때문에 프로젝트를 생성할 때 마다 설정할 필요가 없고 이클립스를 다시 세팅할때 하면된다.
...
//	checkbox는 여러 항목을 선택할 수 있는데 아래와 같이 getParameter() 메소드를 이용해서 넘어오는
//	데이터를 받으면 맨 처음에 선택한 값 1개만 받을 수 있다.
//	데이터가 여러개 넘어오는 경우는 getParameter() 메소드를 사용하면 안된다.
//	String hobbies = request.getParameter("hobbies");
//	out.println(name + "님의 취미는 " + hobbies + " 입니다.<br>");

//	checkbox에서 넘어오는 데이터를 받을 때 항목이 여려개일 수 있기 때문에 getParameterValues()
//	메소드로 받아서 배열에 저장해서 처리한다. => 데이터가 문자열 배열로 넘어온다.
	...
	try {
		...
		out.println("없어요.... 저는 취미가 공부입니다.<br>");
	}
	...
	try {
		...
		out.println("역병이 돌고있는데 가기는 어딜가요<br>");
	}
...
//	태그 사용 가능, 줄바꿈 가능
//	모든 엔터키(\r\n)를 <br> 태그로 바꾸면 줄바꿈이 적용된다.
//	out.println(name + "님이 남긴글 " + content.replace("\r\n", "<br>") + "<br>");
//	태그 사용 불가능, 줄바꿈 가능
//	반드시 replace("\r\n", "<br>")보다 content.replace("<", "&lt;").replace(">", "&gt;")를
//	앞에서 실행해야 한다.
	out.println(name + "님이 남긴글 " + content.replace("<", "&lt;").replace(">", "&gt;")
			.replace("\r\n", "<br>") + "<br>");
%>
