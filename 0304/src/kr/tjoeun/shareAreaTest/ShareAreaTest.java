...
//		기본 자료형으로 선언한 변수는 일반 변수 클래스로 만든 변수(객체, 인스턴스)는 참조 변수라 한다.
//		일반 변수는 데이터 자체를 기억하지만 참조 변수는 생성되는 크기를 특정지을 수 없으므로 
//		클래스로 만드는 변수는 메모리 어딘가에 객체가 생성되면 객체가 생성된 주소를 기억한다. 
		ShareArea shareArea = new ShareArea();
		
//		공유 영역을 만드는 1번째 방법
//		공유 영역을 공유해서 사용할 클래스의 객체를 선언하고 생성된 공유 영역(주소)을 넣어준다.
...
//		공유 영역을 만드는 2번째 방법
//		공유 영역을 공유해서 사용할 클래스의 객체를 선언할 때 생성자의 인수로 공유 영역을
//		넘겨준다.
