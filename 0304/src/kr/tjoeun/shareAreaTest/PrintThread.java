...
//	공유 영역으로 사용할 클래스 객체를 필드로 선언한다.
	...
	public void run() {
		...
		/*
//		CalculatorThread의 실행이 끝날 때 까지(shareArea.ready에 true가 저장될 때 까지)
//		대기시킨다.
...
//		wait() 메소드로 CalculatorThread의 스레드 실행이 끝날 때 까지 대기한다.
//		wait() 메소드는 현재 스레드를 다른 스레드에서 깨울 때 까지 멈춘다.
...
//		쓰면 된다.
		synchronized (shareArea) {
			try { shareArea.wait(); } catch (InterruptedException e) { e.printStackTrace(); }
		}
