...
<%--
	EL(Expression Language, 표현 언어) 내장 객체 => jsp에 표현식(<%= %>)을 대신해서 간단한 데이터 출력에
	사용한다.
	EL 사용법 => ${변수이름} 또는 ${객체이름.변수이름} => getter 메소드를 사용한 것과 같은 효과를 낸다.
	jsp의 println() 메소드를 사용해서 영역 변수에 저장된 내용을 출력할 때 영역 변수에 저장된 데이터가 없으면
	null을 출력하지만 EL은 아무것도 출력하지 않는다.
	
	영역 객체 이름은 영역 이름 뒤에 Scope가 붙는다.
--%>

requestTest.jsp 파일의 pageContext 영역 변수(jsp 표현식 사용): <%=pageContext.getAttribute("pageContentVar")%><br/>
requestTest.jsp 파일의 pageContext 영역 변수(EL 사용): ${pageContextScope.pageContentVar}<br/>
