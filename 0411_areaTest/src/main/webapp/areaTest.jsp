...
//	request     => 현재 보고있는 페이지의 다음 페이지 까지
//	session     => 브라우저가 실행되면 생성되고 브라우저가 종료되면 소멸되는 영역
//	application => 서버가 실행되면 생성되고 서버가 종료되면 소멸되는 영역

	String var = "일반 변수";

//	시스템 영역 변수는 영역이름.setAttribute("영역 변수 이름", 영역 변수에 저장할 데이터)와 같이 만든다.
//	영역 변수가 저장되는 기억 공간은 영역 변수 이름을 key로 하고 영역 변수에 저장되는 데이터를 value로 하는
//	Map<String, Object> 형태의 구조로 value의 자료형이 Object인 이유는 개발자가 어떤 타입의 데이터를 영역 변수에
//	저장할지 모르기 때문이다.
...
//	영역 변수가 저장되는 기억 공간은 Map<String, Object> 형태이므로 getAttribute() 메소드로 영역 변수에
//	저장된 데이터를 얻어오면 Object 타입으로 얻어오기 때문에 반드시 영역 변수에 저장된 데이터는 저장할
//	타입으로 형변환시켜서 일반 변수에 저장해서 사용해야 한다.
...
//	sendRedirect() 메소드는 주소창에 표시되는 페이지 이름이 sendRedirect() 메소드의 인수로 지정한 페이지
//	이름으로 변경된다.
...
//	forward() 메소드는 주소창에 표시되는 페이지 이름이 forward() 메소드의 인수로 지정한 페이지 이름으로
//	변경되지 않고 forward() 메소드가 실행된 페이지 이름이 그대로 유지된다.
//	=> F5를 눌러서 새로고침을 하면 주소창에 표시된 forward() 메소드가 실행된 페이지부터 다시 실행된다.
