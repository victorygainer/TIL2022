...
//		자바의 기본 자료형 => 자바에서 기본적으로 만들어 제공하는 자료형
//		boolean: 1 byte, 논리값(true, false)을 기억한다.
//		byte: 1 byte, 2진 데이터 전송에 사용한다.
//		char: 2 byte, 문자 1개
//		short: 2 byte, -32768 ~ 32767 사이의 정수
//		int: 4 byte, -2147483648 ~ 2147483647 사이의 정수
//		long: 8 byte, -2의 63승 ~ 2의 63승 - 1 사이의 정수
//		float: 4 byte, 단정도 실수, 소수점 아래 6자리 정도를 표현한다.
//		double: 8 byte, 배정도 실수, 소수점 아래 16자리 정도를 표현한다.
...
//		서식있는 출력
//		printf("출력 서식", 출력할 데이터) => JDK 1.5 부터 추가된 내용
//		출력 서식은 출력 서식 문자를 제외한 나머지 내용은 입력한 그대로 출력된다.
//		출력 서식 문자: d(정수), f(실수), s(문자열), c(문자)
//		출력 서식의 형식: %[-][0][n][.m]서식문자
//		n: 출력할 전체 자리수, 출력할 전체 자리수가 지정되면 오른쪽에 맞춰 출력된다.
//		-: 출력할 전체 자리수가 지정된 경우 왼쪽으로 맞춰 출력한다.
//		0: 출력할 전체 자리수가 지정된 경우 왼쪽의 남는 자리를 '0'을 채워 출력한다.
//		.m: 출력할 소수점 아래 자리수를 지정한다. 잘리는 자리에서 반올림해서 화면에 표시한다.
...
//		발생되니 주의한다.
//		System.out.printf("평균점수: %6.2f", 100); // 예외발생
