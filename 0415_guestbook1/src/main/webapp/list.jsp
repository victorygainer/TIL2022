...
//	검색어가 넘어온 경우(내용, 이름, 내용+이름)와 넘어오지 않은 경우에 각각 메소드를 실행한다.
	...
	/*
//	검색어가 없는 경우와 검색어가 내용, 이름, 내용+이름인 경우 각각 메소드로 구현했다.
	if (item == null || item.trim().length() == 0) {
		// 검색어가 입력되지 않은 경우
		guestbookList = select.selectList(currentPage);
	} else if (category.equals("내용")) {
		// 검색어가 입력되고 카테고리가 "내용"인 경우
		guestbookList = select.selectListMemo(currentPage, item);
	} else if (category.equals("이름")) {
		// 검색어가 입력되고 카테고리가 "이름"인 경우
		guestbookList = select.selectListName(currentPage, item);
	} else if (category.equals("내용+이름")) {
		// 검색어가 입력되고 카테고리가 "내용+이름"인 경우
		guestbookList = select.selectListMemoName(currentPage, item);
	}
	*/

//	검색어가 없는 경우와 검색어가 있는 경우로 메소드를 구현한다.
	if (item == null || item.trim().length() == 0) {
		// 검색어가 입력되지 않은 경우
		guestbookList = select.selectList(currentPage);
	} else {
