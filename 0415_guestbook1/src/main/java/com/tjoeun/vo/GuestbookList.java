...
//	1페이지 분량의 글(GuestboobVO) 목록을 기억할 List를 선언한다.
...
//	페이지 작업에 사용할 8개의 변수를 선언한다. => 계산에 의한 값을 가져야하는 변수는 0으로 초기화 한다.
	...
	private int startNo = 0; // 현재 브라우저에 표시되는 글의 시작 인덱스 번호 => mysql은 index가 0부터 시작된다.
...
//	pageSize, totalCount, currentPage를 인수로 넘겨받아 초기화시키고 나머지 변수에 초기화될 값을 계산해서 
//	초기화 하는 생성자
...
//	pageSize, totalCount, currentPage를 제외한 나머지 변수를 계산하는 메소드
	private void calculator() {
		totalPage = (totalCount - 1) / pageSize + 1;
		currentPage = currentPage > totalPage ? totalPage : currentPage;
		// oracle은 select sql 명령 실행결과 index 값이 1부터 시작되므로 mysql에 사용했던 계산식에 1을
		// 더해야 한다.
