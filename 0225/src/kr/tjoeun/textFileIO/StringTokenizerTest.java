...
//		구분자를 생략하면 공백과 탭을 기본 구분자로 사용하며 구분자로 구분된 데이터를
//		token이라고 부른다.
		
//		문자열이 공백이나 탭으로 구분된 경우
		String str1 = "사과 배 복숭아	밤 대추";
		StringTokenizer st1 = new StringTokenizer(str1);
//		hasMoreTokens() 메소드는 StringTokenizer 객체에 다음에 읽을 token이 있으면 true,
//		없으면 false를 리턴한다.
		while (st1.hasMoreTokens()) { // 다음에 읽을 데이터(token)이 있는동안 반복한다.
//			nextToken() 메소드는 StringTokenizer 객체의 다음 데이터를 읽어들인다.
...
//		StringTokenizer 클래스 생성자의 3번째 인수로 구분자를 token에 포함여부를 
//		지정할 수 있다. 생략시 기본값은 false로 구분자를 토큰에 포함시키지 않는다.  
