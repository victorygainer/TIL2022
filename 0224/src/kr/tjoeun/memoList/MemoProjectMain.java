...
//	데이터를 입력받아 memoList라는 ArrayList에 저장된 글 1건을 수정하는 메소드
...
//		memoList라는 ArrayList에 저장된 수정할 글 1건을 얻어오는 메소드를 실행해서 얻어온 결과를
//		MemoVO 클래스 객체에 저장한다.
		MemoVO original = memoList.selectMemo(idx);
		
//		수정할 글번호에 해당되는 글이 존재하면 글을 수정하고 존재하지 않으면 글이 존재하지 않는다는
//		메시지를 출력한다.
		if (original == null) {
...
//	memoList라는 ArrayList에 저장된 글 1건을 삭제하는 메소드
...
//		memoList라는 ArrayList에 저장된 삭제할 글 1건을 얻어오는 메소드를 실행해서 얻어온 결과를
//		MemoVO 클래스 객체에 저장한다.
		MemoVO original = memoList.selectMemo(idx);
		
//		삭제할 글번호에 해당되는 글이 존재하면 글을 삭제하고 존재하지 않으면 글이 존재하지 않는다는
//		메시지를 출력한다.
...
//			비교한다.
//			비밀번호가 일치하면 memoList라는 ArrayList에 저장된 글을 삭제하는 메소드를 실행하고
//			비밀번호가 일치하지 않으면 비밀번호가 일치하지 않는다는 메시지를 출력한다.
			if (password.equals(original.getPassword())) {
//				비밀번호가 일치하므로 ArrayList에 저장된 글을 삭제하는 메소드를 실행한다.
...
//		입력받은 데이터를 MemoVO 클래스 객체에 저장한다.
		MemoVO vo = new MemoVO(name, password, memo);
//		System.out.println(vo);
//		System.out.println("MemoVO.count: " + MemoVO.count);
