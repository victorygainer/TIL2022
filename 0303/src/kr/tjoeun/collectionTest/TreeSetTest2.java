...
//		향상된 for를 사용해서 TreeSet 이나 TreeSet 객체의 데이터를 1개씩 얻어올 수 있다.
		for (String str : tset) {
			list.add(str);
		}
...
//		Iterator 인터페이스는 TreeSet 이나 TreeSet과 같이 입력하는 순서와 저장되는 순서가 달라서
//		get(), set() 메소드를 사용할 수 없는 객체를 분리한다.
//		TreeSet, TreeSet 객체에서 iterator() 메소드를 실행하면 데이터 단위로 분리한다.
