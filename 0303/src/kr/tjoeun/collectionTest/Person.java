...
//	객체의 정렬 기준을 지정해야 한다.
//	TreeSet에 저장할 클래스에 구현한 Comparable 인터페이스의 제네릭에는 현재 클래스 이름을 넣는다.
...
//		compareTo() 메소드는 자신(this)과 인수(Person person)로 넘어온 객체(add() 메소드의 인수로 지정한
//		객체)에 저장된 데이터를 비교한다.
//		비교할 데이터가 문자열일 경우 compareTo() 메소드는 자신이 크면 1, 같으면 0, 작으면 -1을 리턴한다.
//		비교할 데이터가 숫자일 경우 "-" 연산을 실행해서 자신이 크면 양수, 같으면 0, 작으면 음수를 리턴한다.
//		return this.name.compareTo(person.name); // name의 오름차순
//		return -this.name.compareTo(person.name); // name의 내림차순
//		return this.age - person.age; // age의 오름차순
//		return person.age - this.age; // age의 내림차순
		
//		name을 기준으로 오름차순 정렬한다. 단, name이 같으면 age를 내림차순으로 정렬한다.
		if (this.name.compareTo(person.name) == 0) { // name이 같은가?
//			age의 내림차순 정렬
